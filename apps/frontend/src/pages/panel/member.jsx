...
import { FaPlus, FaUser, FaTrash, FaPencilRuler } from "react-icons/fa";
...
function Member() {
  ...
  const handleDelete = async (id) => {
    ...
    axios.delete(`/v1/restricted/member/${id}`, {
      headers: {
        Authorization: `Bearer ${getCookie("token")}`,
      },
    }).then((response) => {
            ...
            <div className="modal-action flex justify-between gap-2">
              ...
              onClick={handleEdit}
               className="text-white p-3 rounded-xl bg-green-600 shadow-lg transition-all duration-200 hover:bg-green-700 hover:scale-105 active:scale-95 focus:ring-2 focus:ring-gray-300">
                {loading ? (<span class="loading loading-spinner loading-md"></span>) : "ยืนยัน"}
              </button>
            </div>
          </div>
        </dialog>
      ...
      <div className="mx-auto  max-w-6xl p-5 bg-white shadow-lg min-h-[70vh] rounded-lg">
        ...
        <div className="overflow-x-auto border border-gray-300 rounded-xl">
          <table className="table text-black text-center min-w-full">
            <thead>
              <tr className="border-b  border-gray-300 text-black text-[13px] ">
